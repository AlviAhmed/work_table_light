...
#define redpress (~PINB) & (_BV(0)) //PB0 with pinup resistor
#define partpress (~PINB) & (_BV(1)) //PB1  with pinup resistor

unsigned volatile int redbut = 0; //last button press for red button (PB0)    
unsigned volatile int redenable = 0;
unsigned volatile int partbut = 0;//last button press for part button (PB1)
unsigned volatile int partenable = 0; 
unsigned volatile int potenable = 1;
int main (void) {   
	init(); 
	pinint();
	while (1){    
